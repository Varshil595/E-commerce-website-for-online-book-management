<?php
	...
	if(isset($_POST) & !empty($_POST)){
		...
		if(isset($_FILES) & !empty($_FILES)){
			...
			if(isset($name) && !empty($name)){
				if(($extension == "jpg" || $extension == "jpeg") && $type == "image/jpeg" && $size<=$max_size){
					...
					if(move_uploaded_file($tmp_name, $location.$name)){
						...
						$sql = "INSERT INTO books (name, description, catid, price, thumb) VALUES ('$prodname', '$description', '$category', '$price', '$location$name')";
			...
			$sql = "INSERT INTO books (name, description, catid, price) VALUES ('$prodname', '$description', '$category', '$price')";
			$res = mysqli_query($connection, $sql);
			if($res){
				header('location: products.php');
			}else{
